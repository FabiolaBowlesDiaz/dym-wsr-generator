"""
Script para debuggear el cálculo de PY 2025 para Campos de Solana
...
print("="*80)
print("ANÁLISIS PY 2025 PARA CAMPOS DE SOLANA - DÍA 27 (SEMANA 4)")
print("="*80)
print()

# 1. VENTAS REALES POR CIUDAD (S1, S2, S3) - SIN Oruro/Trinidad
print("1. VENTAS REALES CAMPOS DE SOLANA POR CIUDAD (Semanas 1, 2, 3) - Ciudades con Gerente")
print("-"*80)
...
WHERE anio = 2025 AND mes = 10 AND dia <= 27
    AND UPPER(marcadir) = 'CAMPOS DE SOLANA'
...
if not df_ventas.empty:
    ...
    print("No hay ventas reales para Campos de Solana en ciudades con gerente")
    print()
print()
print()

# 2. PROYECCIONES GERENTES POR CIUDAD (S4, S5)
print("2. PROYECCIONES GERENTES CAMPOS DE SOLANA POR CIUDAD (Semanas 4 y 5)")
print("-"*80)
...
WHERE anio_proyeccion = 2025 AND mes_proyeccion = 10
    AND UPPER(nombre_marca) = 'CAMPOS DE SOLANA'
    AND UPPER(ciudad) NOT IN ('ORURO', 'TRINIDAD', 'TURISMO')
...
if not df_proy.empty:
    ...
    print("No hay proyecciones de gerentes para Campos de Solana")
    print()
print()
print()
...
WHERE EXTRACT(YEAR FROM CAST(tiempo_key AS DATE)) = 2025
    ...
    AND UPPER(marcadirectorio) = 'CAMPOS DE SOLANA'
...
if not df_sop.empty:
    ...
    print("No hay datos de SOP para Oruro/Trinidad con Campos de Solana")
    print()
...
if not df_ventas_ot.empty:
    ...
    print("No hay ventas reales para Oruro/Trinidad con Campos de Solana")
    print()
...
# 5. CÁLCULO FINAL
print("="*80)
print("CÁLCULO FINAL PY 2025 CAMPOS DE SOLANA")
print("="*80)
print()

total_ventas_reales = df_ventas['total_ventas_reales'].sum() if not df_ventas.empty else 0
...
print()
print("-"*80)
print(f"PY 2025 CAMPOS DE SOLANA TOTAL (Manual):  {total_ventas_reales + total_proy_gerentes + total_ventas_ot + total_sop_ot:>15,.2f} BOB")
print("="*80)
print()
...
campos = result[result['marcadir'].str.upper() == 'CAMPOS DE SOLANA']

if not campos.empty:
    py_query = campos['py_2025_bob'].values[0]
    ...
    if abs(py_query - py_manual) < 0.01:
        print("✓ CORRECTO - Coinciden!")
    else:
        print("✗ ERROR - No coinciden!")
else:
    print("No se encontró Campos de Solana en el resultado del query")
